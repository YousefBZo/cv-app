...
<template>
  <FormCard>
    ...
    <div v-else-if="hasProfile === false" class="text-center py-12 space-y-4">
      ...
      <p class="text-slate-400 text-sm">{{ t('forms.profileRequiredText', { section: t('sidebar.volunteer').toLowerCase() }) }}</p>
    ...
    <template v-else>
    <SectionHeader icon="🤝" :title="t('forms.addVolunteer')" :subtitle="t('forms.volunteerSubtitle')"
    ...
    <form @submit.prevent="handleSubmit" class="space-y-5">
      <div>
        ...
        <input type="text" id="organization" v-model="formData.organization" :placeholder="t('forms.organizationVolPlaceholder')" class="input-dark" />
        <p v-if="errors.organization" class="text-red-400 text-xs mt-1.5">{{ errors.organization[0] }}</p>
      </div>

      <div>
        <label for="role" class="label-dark">{{ t('forms.role') }}</label>
        <input type="text" id="role" v-model="formData.role" :placeholder="t('forms.rolePlaceholder')" class="input-dark" />
      ...
      <div>
        ...
        <textarea id="description" v-model="formData.description" rows="3" :placeholder="t('forms.descriptionVolPlaceholder')" class="input-dark resize-none"></textarea>
      ...
      <button type="submit" :disabled="loading" class="btn-primary flex items-center justify-center gap-2">
        ...
        {{ loading ? t('forms.saving') : t('forms.saveVolunteer') }}
      </button>
    </form>
    </template>
  </FormCard>
