  ...
  <section class="max-w-4xl mx-auto py-8 sm:py-12 px-4 sm:px-6">
    ...
    <div v-if="projects.length" class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div v-for="project in projects" :key="project.id"
        ...
        class="group bg-black/5 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden hover:bg-white/8 hover:border-blue-500/40 transition-all duration-500 cursor-pointer">
        <div v-if="project.cover" class="w-full h-32 sm:h-44 overflow-hidden">
          <img :src="project.cover" :alt="project.title"
            class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        </div>
        <div v-else class="w-full h-32 sm:h-44 bg-slate-100/50 dark:bg-slate-100 dark:bg-slate-800/50 flex items-center justify-center">
        ...
        <div class="p-4 sm:p-6">
          <div class="flex flex-wrap justify-between items-start gap-2 mb-3 sm:mb-4">
            <h3 class="text-lg sm:text-xl font-bold text-slate-900 dark:text-white group-hover:text-blue-400 transition-colors">{{ project.title }}</h3>
            <div class="flex gap-2" @click.stop>
              <a v-if="project.github_url" :href="project.github_url" target="_blank" class="text-slate-500 hover:text-slate-900 dark:text-white transition-colors" title="GitHub">
              ...
              <a v-if="project.link" :href="project.link" target="_blank" class="text-slate-500 hover:text-slate-900 dark:text-white transition-colors" title="Live Demo">
