...
<template>
  <section class="max-w-4xl mx-auto py-8 sm:py-12 px-4 sm:px-6">
    <div class="flex items-center gap-4 mb-8 sm:mb-12">
      <h2 class="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-white">{{ t('cv.skills') }}</h2>
    ...
    <div v-if="skills.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="(skill, idx) in skills" :key="idx"
        ...
        class="bg-black/5 dark:bg-white/5 p-5 rounded-2xl border border-slate-200 dark:border-white/10 cursor-pointer hover:bg-white/8 hover:border-blue-500/30 transition-all">
        <div class="flex justify-between items-end mb-4">
          <div>
            <div class="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-1">{{ skill.level ?? 'Proficient' }}</div>
            <div class="text-lg font-bold text-slate-900 dark:text-white">{{ skill.name }}</div>
          </div>
        </div>
