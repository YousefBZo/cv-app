...
<template>
  <section class="max-w-4xl mx-auto py-8 sm:py-12 px-4 sm:px-6">
    <div class="flex items-center gap-4 mb-8 sm:mb-12">
      <h2 class="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-white">{{ t('cv.languagesTitle') }}</h2>
    ...
    <div v-if="languages.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="lang in languages" :key="lang.id"
        ...
        class="bg-black/5 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-5 rounded-2xl hover:bg-white/8 hover:border-purple-500/30 transition-all duration-300 cursor-pointer">
        <div class="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ lang.name }}</h3>
            <p class="text-xs font-semibold uppercase tracking-wider" :class="{
              ...
              'text-slate-600 dark:text-slate-400': getLangLevel(lang.level).color.includes('slate'),
