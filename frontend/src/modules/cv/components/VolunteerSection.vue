...
<template>
  <section class="max-w-4xl mx-auto py-12 px-6">
    ...
    <div class="mt-12 flex justify-center gap-4">
      <button v-if="hasMore" @click="emit('showAll')" class="px-8 py-3 rounded-full border border-slate-700 text-slate-400 hover:text-teal-400 hover:border-teal-500/30 transition-all text-xs font-bold uppercase tracking-widest">View More</button>
      <button v-if="isExpanded" @click="emit('showLess')" class="px-8 py-3 rounded-full border border-slate-700 text-slate-500 hover:text-teal-400 transition-all text-xs font-bold uppercase tracking-widest">− Show Less</button>
    </div>
  </section>
</template>
