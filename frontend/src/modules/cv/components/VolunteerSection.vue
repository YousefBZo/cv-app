...
<template>
  <section class="max-w-4xl mx-auto py-8 sm:py-12 px-4 sm:px-6">
    <div class="flex items-center gap-4 mb-8 sm:mb-12">
      <h2 class="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-white">{{ t('cv.volunteerTitle') }}</h2>
    ...
    <div v-if="volunteers.length" class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div v-for="vol in volunteers" :key="vol.id"
        @click="emit('edit', vol)"
        class="bg-black/5 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-4 sm:p-6 rounded-2xl shadow-lg hover:bg-white/8 hover:border-teal-500/30 transition-all duration-300 cursor-pointer">
        <div class="flex items-start gap-4">
          ...
          <div>
            ...
            <h3 class="text-lg sm:text-xl font-bold text-slate-900 dark:text-white mb-1">{{ vol.role }}</h3>
            <p class="text-slate-700 dark:text-slate-300 font-medium">{{ vol.organization }}</p>
    ...
    <div class="mt-12 flex justify-center gap-4">
      <button v-if="hasMore" @click="emit('showAll')" class="px-5 sm:px-8 py-2 sm:py-3 rounded-full border border-slate-700 text-slate-600 dark:text-slate-400 hover:text-teal-400 hover:border-teal-500/30 transition-all text-xs font-bold uppercase tracking-widest">{{ t('cv.viewMoreVolunteer') }}</button>
