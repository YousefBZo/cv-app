...
<template>
  <section class="max-w-4xl mx-auto py-8 sm:py-12 px-4 sm:px-6">
    <div class="flex items-center gap-4 mb-8 sm:mb-12">
      <h2 class="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-white">{{ t('cv.experience') }}</h2>
    ...
    <div v-if="experiences.length" class="relative border-l border-slate-700 ml-3 space-y-8 sm:space-y-12">
      <div v-for="exp in experiences" :key="exp.id" class="relative pl-6 sm:pl-8 group cursor-pointer"
        @click="emit('edit', exp)">
        <div class="absolute -left-2.25 top-2 w-4 h-4 rounded-full bg-white dark:bg-slate-900 border-2 border-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]"></div>
        <div class="bg-black/5 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-4 sm:p-6 rounded-2xl shadow-xl hover:bg-white/8 hover:border-blue-500/30 transition-all">
          <div class="flex flex-wrap justify-between items-start gap-2 mb-3 sm:mb-4">
            <div>
              <h3 class="text-lg sm:text-xl font-bold text-slate-900 dark:text-white">{{ exp.position }}</h3>
          ...
          <p class="text-slate-600 dark:text-slate-400 leading-relaxed text-sm whitespace-pre-line">{{ exp.description }}</p>
        </div>
      </div>
    </div>
